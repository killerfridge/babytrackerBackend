...
@router.get('/{baby_id}/plot', response_model=List[schemas.TempPlot])
def get_plot(baby_id: int, db: Session = Depends(get_db), user: schemas.User = Depends(get_current_user)):
    ...
    temp = db.query(models.Temperature)\
        .filter(models.Temperature.baby_id == baby_id)\
        .order_by(models.Temperature.created_at.desc())\
        .all()

    if not temp:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"No temperature data for {baby_id}")
